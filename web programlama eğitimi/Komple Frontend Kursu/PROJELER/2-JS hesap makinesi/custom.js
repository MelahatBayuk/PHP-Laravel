let dugme=document.getElementsByTagName("button");
//console.log(dugme);16 buton varmış
let ekran=document.getElementsByClassName("ekran1")[0];
let hesapla=document.getElementById("hesapla");
let temizle=document.getElementById("temizle");
for(var i=0; i<16; i++){
    if(i!=12 && i!=14){
        dugme[i].addEventListener("click",yazdir);
    }   
}
function yazdir(){
  ekran.value=ekran.value + this.value; //hangi tuşa basıldıysa onu getirmesi için this kullanıldı

}
hesapla.addEventListener("click",hesap);
function hesap(){
    ekran.value=eval(ekran.value); //eval ekranın değerni eval ile hesaplayıp tekrar içine at
}
temizle.addEventListener("click",temiz);
function temiz(){
    ekran.value="";
}